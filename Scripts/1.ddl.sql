...
CREATE TABLE `proj21_shop`.`MEMBER` (
	...
	`m_birthday`     VARCHAR(100) NOT NULL COMMENT '생년월일', -- 생년월일
	`m_gender`       TINYINT      NOT NULL COMMENT '성별', -- 성별
	`m_addr1`		 VARCHAR(500) NOT null COMMENT '우편번호',
	`m_addr2`		 VARCHAR(500) NOT null COMMENT '주소',
	`m_addr3`		 VARCHAR(500) NOT null COMMENT '상세주소',
	`m_question`     VARCHAR(1000) NOT NULL COMMENT '본인확인질문번호', -- 본인확인질문번호
	`m_answer`       varchar(500) NOT NULL COMMENT '본인확인답변', -- 본인확인답변
	`m_email`        VARCHAR(500) NULL     COMMENT '이메일', -- 이메일
	`m_join`         DATETIME     NULL     default current_timestamp COMMENT 'default current_timestamp', -- 가입일
	`m_point`        INT          NULL     default 0 COMMENT 'default 0', -- 포인트
	`m_total_buy`    INT          NULL     default 0 COMMENT 'default 0', -- 누적사용액
	`m_total_order`  INT          NULL     default 0 COMMENT 'default 0', -- 누적주문수
	`m_total_login`  INT          NULL     default 0 COMMENT 'default 0', -- 로그인횟수
	`m_exit`         TINYINT      NULL     default 0 COMMENT 'default 0' -- 탈퇴여부
)
COMMENT '회원';
...
CREATE TABLE `proj21_shop`.`QNA` (
	...
	`q_date`    DATETIME      NULL     default current_timestamp COMMENT 'default current_timestamp', -- 질문일
	`q_hits`    INT           NULL     default 0 COMMENT  'default 0', -- 질문조회수
	`q_group`   INT           NOT NULL COMMENT '질문그룹', -- 질문그룹
	`q_indent`  INT           NULL     default 0 COMMENT  'default 0', -- 질문레벨
	`q_step`    INT           NULL     default 0 COMMENT  'default 0' -- 질문순서
)
COMMENT 'Q&A';
...
CREATE TABLE `proj21_shop`.`REVIEW` (
	...
	`re_date`       DATETIME      NULL     default current_timestamp COMMENT 'default current_timestamp', -- 후기일
	`re_replyCount` INT           NULL     default 0 COMMENT '후기수', -- 후기수
...
CREATE TABLE `proj21_shop`.`RE_REPLY` (
	...
	`re_rep_date`   DATETIME     NULL     default current_timestamp COMMENT 'default current_timestamp' -- 덧글일
)
COMMENT '후기덧글';
...
CREATE TABLE `proj21_shop`.`PRODUCT` (
	...
	`pro_content`    VARCHAR(100) NULL     COMMENT '제품설명', -- 제품설명
	`pro_salesrate`  INT          NULL     COMMENT '제품세일', -- 제품세일
	`pro_cre_date`   DATETIME     NULL     default current_timestamp COMMENT 'default current_timestamp', -- 제품생성일
	`pro_status`     VARCHAR(50)  NULL     default '판매중' COMMENT '제품상태', -- 제품상태
	...
	`pro_hits`       INT          NULL     default 0 COMMENT 'default 0' -- 조회수
)
COMMENT '제품';
...
CREATE TABLE `proj21_shop`.`PRO_IMG` (
	...
	`pro_img_state`     TINYINT      NULL     default 0 COMMENT 'default 0' -- 메인이미지여부
)
COMMENT '제품이미지';
...
CREATE TABLE `proj21_shop`.`ORDER` (
	...
	`delivery_status`     VARCHAR(100)  NULL     default '배송준비중' COMMENT 'default(배송준비중)', -- 배송상태
	`request_to_delivery` VARCHAR(1000) NULL     COMMENT '배송참고사항', -- 배송참고사항
	`order_date`          DATETIME      NOT NULL default current_timestamp COMMENT 'default current_timestamp', -- 주문일
